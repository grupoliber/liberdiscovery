...
export default function AlertListWidget({ config, lastRefresh, selectedHost }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (selectedHost) {
            data = data.filter(a => a.hosts?.some(h => h.hostid === selectedHost));
          }
          setAlerts(data);
        }
      } catch {}
    ...
    load();
  }, [limit, lastRefresh, selectedHost]);
        ...
        <div key={a.eventid} className="flex items-start gap-2 bg-dark-800 rounded p-2">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              {a.hosts?.[0] && <span className="text-[10px] text-dark-400">{a.hosts[0].name}</span>}
              <span className="text-[10px] text-dark-500">{new Date(parseInt(a.clock) * 1000).toLocaleString('pt-BR')}</span>
            </div>
          </div>
        </div>
      ))}
